...
7. 若问题是计算类（比例、差值、同比等），`fields` 应包含参与计算的原始字段，不要虚构派生字段。**但**：数据库已经存了多个预算好的同比列（`*_yoy_growth`），如 `operating_revenue_yoy_growth`、`net_profit_yoy_growth`、`asset_total_assets_yoy_growth` 等。当用户问的是**多期/跨年**的同比增长率趋势时，直接把这些预算列放进 `fields`，`yoy` 保持 false，`is_trend` 置 true。
8. `yoy` 仅用于**单期**同比计算（JOIN 当期 vs 上年同期，适合"2024年净利润同比是多少"这种单点问）。多期趋势/多公司同比排名请走预算字段路径，不要设 yoy=true。
...
- 营业收入同比增长率 -> income_sheet.operating_revenue_yoy_growth 或 core_performance_indicators_sheet.operating_revenue_yoy_growth
- 净利润同比增长率 -> income_sheet.net_profit_yoy_growth 或 core_performance_indicators_sheet.net_profit_yoy_growth
- 总资产/总负债同比增长率 -> balance_sheet.asset_total_assets_yoy_growth / balance_sheet.liability_total_liabilities_yoy_growth
- 净现金流同比增长率 -> cash_flow_sheet.net_cash_flow_yoy_growth
...
Q: 白云山2022年至2025年营业总收入同比增长率
A: {{"tables":["income_sheet"],"fields":["operating_revenue_yoy_growth"],"companies":["白云山"],"periods":[],"is_trend":true,"yoy":false,"top_n":null,"order_direction":null}}

Q: 华润三九近几年净利润同比增长率变化
A: {{"tables":["income_sheet"],"fields":["net_profit_yoy_growth"],"companies":["华润三九"],"periods":[],"is_trend":true,"yoy":false,"top_n":null,"order_direction":null}}
