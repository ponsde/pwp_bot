...
- [ ] 4.3 实现上交所文件名解析：stock_code 从文件名取，report_period 优先从 PDF 首页标题解析（如"2023年年度报告"→2023FY），兜底用发布日期推断
- [ ] 4.4 实现同日期多 PDF 过滤：上交所同一日期有多份 PDF（正文/摘要/审计），按文件大小或首页标题过滤出完整报告
- [ ] 4.5 实现跨页表格合并：处理合并单元格产生的空列清洗、跨页表格拼接
- [ ] 4.6 实现核心业绩指标提取：从 PDF 前 15 页（"主要会计数据和财务指标"部分）提取 EPS、营收、净利润等
- [ ] 4.7 测试：华润三九 2023 年报 PDF 能提取出 4 类表格（核心业绩、资产负债表、利润表、现金流量表）
