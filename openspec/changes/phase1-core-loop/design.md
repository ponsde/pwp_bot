...
**实测发现**（2026-03-22 华润三九 2023 年报）：
- pdfplumber 能提取表格，但有大量空列（合并单元格产生的），需要清洗
- 合并资产负债表在 p100-103，合并利润表在 p103-105（同页会有两个表），合并现金流量表在 p106
- 核心业绩指标在 p7（"主要会计数据和财务指标"部分），不在后面的财务报表中
- 数值单位为元（含千分位逗号如 "24,738,963,319.76"），需去逗号 + 转万元
- 表格跨多页，需要拼接
...
**识别策略**（优先级从高到低）：
1. 解析 PDF 第一页标题（如"2023年年度报告"→ 2023FY）—— 实测上交所 PDF 首页明确写了报告类型
2. 文件大小判断：同一日期多份 PDF，最大的通常是正文（排除"摘要"/"审计报告"）
3. 日期推断兜底：3-4月→上年FY，4-5月→Q1，7-8月→HY，10-11月→Q3

**实测发现**（2026-03-22）：
- 上交所同一日期有多份 PDF（如 20230428 有 3 份：年报正文 4MB、摘要 500KB 等）
- 需要过滤掉报告摘要，只处理完整报告
