...
def prepare_channel(
    ...
    if enable_openapi and fastapi_app is not None:
        ...
        # Read api_key from ov.conf channels[type=openapi] if present
        _openapi_api_key = ""
        for _ch in config.channels:
            if isinstance(_ch, dict) and _ch.get("type") == "openapi" and _ch.get("api_key"):
                _openapi_api_key = _ch["api_key"]
                break
